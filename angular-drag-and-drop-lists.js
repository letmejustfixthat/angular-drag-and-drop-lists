        ...
        function($parse,   $timeout,   dndDropEffectWorkaround,   dndDragTypeWorkaround, dndDragUtil) {
            return function(scope, element, attr) {
                ...
                function isMouseInFirstHalf(event, targetNode, relativeToParent) {
                    ...
                    var elemBox = targetNode.getBoundingClientRect();
                    var bodyBox = document.body.getBoundingClientRect();
                    var offsets = {
                        left: elemBox.left - bodyBox.left,
                        right: bodyBox.right - elemBox.right,
                        top: elemBox.top - bodyBox.top,
                        bottom: bodyBox.bottom - elemBox.bottom
                    }
                    var targetStart = (horizontal) ? offsets.left : offsets.top;
                    var targetSize = (horizontal) ? elemBox.right - elemBox.left : elemBox.bottom - elemBox.top;
                    var mousePointer = (horizontal) ? event.pageX : event.pageY;
                    return mousePointer < targetStart + (targetSize/2);
                }
